...
void CorrespondenceGraph::setTargets(const std::vector<Point>& targets)
{
  targets_.clear();
...
void CorrespondenceGraph::setInputs(const std::vector<Point>& inputs)
{
  inputs_.clear();
...
void CorrespondenceGraph::computeTransform(Transformation& transf,std::vector<std::pair<int,int> >& associations)
{
  Transformation inputTtarget;
  associate(associations);
  computeTransform(inputs_,targets_,associations,inputTtarget);
//  transf = inputTtarget.inverse();
  transf = inputTtarget; 
}
...
void CorrespondenceGraph::makeNodeSet(const std::vector<Point>& points1,const std::vector<Point>& points2,
...
void CorrespondenceGraph::makeRelativeConstraints(const std::vector<Point>& points,std::vector<Constraint>& constraints)
{
...
bool CorrespondenceGraph::computeTransform(const std::vector<Point>& points1,const std::vector<Point>& points2,
